        ...
        <label class="block">
            ...
            <div class="relative">
                ...
                <input
                    ...
                    placeholder="Busque por codigo, endereco, descricao ou responsavel"
                >
            </div>
        </label>
    </div>
                        ...
                        <tr class="bg-layer">
                            <td class="px-5 py-4">
                                <div>
                                    ...
                                    @else
                                        <p class="text-sm text-muted-foreground">Sin direccion informado</p>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-4 text-sm text-foreground">
                                {{ $service->user?->name ?? 'Sin responsavel' }}
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                @php($statusClasses = [
                                    ...
                                    'en_andamiento' => 'bg-sky-100 text-sky-700',
                                    'cerrado' => 'bg-emerald-100 text-emerald-700',
                                ])
                                <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold {{ $statusClasses[$service->status] ?? 'bg-slate-100 text-slate-700' }}">
                ...
                <div class="relative z-10 w-full max-w-4xl rounded-2xl border border-layer-line bg-layer shadow-xl">
                    <div class="flex items-center justify-between border-b border-layer-line px-6 py-4">
                        <div>
                            <h3 class="text-lg font-semibold text-foreground">
                                {{ $editingServiceId ? 'Editar servicio' : 'Novo servicio' }}
                            </h3>
                            <p class="mt-1 text-sm text-muted-foreground">
                                {{ $editingServiceId ? 'Atualize os dados do servicio selecionado.' : 'Preencha os dados para criar um novo servicio.' }}
                            </p>
                        </div>
                    ...
                    <form wire:submit="save" class="space-y-5 px-6 py-6">
                        <div class="grid gap-5 md:grid-cols-2">
                            ...
                            <label class="block">
                                <span class="mb-2 block text-sm font-medium text-foreground">Codigo</span>
                                <input
                                    type="text"
                            ...
                            <label class="block md:col-span-2">
                                <span class="mb-2 block text-sm font-medium text-foreground">Endereco</span>
                                <input
                                    type="text"
                                    wire:model.live="address"
                                    class="block w-full rounded-lg border border-layer-line bg-surface px-4 py-3 text-sm text-foreground focus:border-primary-focus focus:outline-hidden focus:ring-0"
                                    placeholder="Digite o endereco do servicio"
                                >
                                @error('address')
                                    <span class="mt-2 block text-sm text-red-600">{{ $message }}</span>
                                @enderror
                            ...
                            <label class="block">
                                <span class="mb-2 block text-sm font-medium text-foreground">Codigo postal</span>
                                <input
                                    type="text"
                            ...
                            <label class="block">
                                ...
                                >
                                    ...
                                    <option value="en_andamiento">En Andamiento</option>
                            ...
                            <label class="block">
                                <span class="mb-2 block text-sm font-medium text-foreground">Data inicial</span>
                                <input
                                    type="date"
                            ...
                            <label class="block">
                                <span class="mb-2 block text-sm font-medium text-foreground">Data final</span>
                                <input
                                    type="date"
                            ...
                            <label class="block md:col-span-2">
                                <span class="mb-2 block text-sm font-medium text-foreground">Descricao</span>
                                <textarea
                                    wire:model.live="description"
                                    rows="4"
                                    class="block w-full rounded-lg border border-layer-line bg-surface px-4 py-3 text-sm text-foreground focus:border-primary-focus focus:outline-hidden focus:ring-0"
                                    placeholder="Descreva o servicio"
                                ></textarea>
                                @error('description')
                                    <span class="mt-2 block text-sm text-red-600">{{ $message }}</span>
                                @enderror
                        ...
                        <div class="flex flex-col-reverse gap-3 border-t border-layer-line pt-5 sm:flex-row sm:justify-end">
                            ...
                            >
                                {{ $editingServiceId ? 'Salvar alteracoes' : 'Criar servicio' }}
                            </button>
                        </div>
                    </form>
                </div>
            ...
            <div class="flex min-h-full items-center justify-center p-4">
                <div class="relative z-10 w-full max-w-lg rounded-2xl border border-layer-line bg-layer shadow-xl">
                    <div class="border-b border-layer-line px-6 py-4">
                        <h3 class="text-lg font-semibold text-foreground">Excluir servicio</h3>
                        <p class="mt-1 text-sm text-muted-foreground">
